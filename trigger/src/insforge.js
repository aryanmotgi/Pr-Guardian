...
async function getClient() {
	...
	// ASSUMPTION: INSFORGE_ANON_KEY is the public anon key from `npx @insforge/cli metadata`
	return createClient({
...
// Returns rule description strings. Falls back to hardcoded list if Insforge
// is not configured or the rules table is empty / unreachable.
...
async function fireViolationEvent(violation, pr) {
	...
	try {
		// ASSUMPTION: realtime.connect() must be called before publish(); safe to call multiple times
		await client.realtime.connect();
		// ASSUMPTION: subscribe() required before publish() per SDK docs; RLS may need to permit anon writes
...
module.exports = { getRules, fireViolationEvent };
