...
async function route(pr, decision) {
	...
	if (verdict === "violation" && confidence === "high") {
		console.log(
			`ACTION: Handing off to fix engine — PR #${pr.number} in ${file}`,
		);
		...
		console.log(
			`ACTION: Escalating PR #${pr.number} to human — confidence too low or unsure`,
		);
	}
}
...
async function handoffToFixEngine(pr, decision) {
	...
	if (!fixUrl) {
		console.warn(
			"FIX_ENGINE_URL not set — skipping POST (payload logged above)",
		);
		return;
	}
