...
async function runFromContract(pr = {}, result = {}) {
	...
	let outcome =
		result.outcome || (result.escalate === true ? "escalate" : "fix");
	if (!VALID.includes(outcome)) {
		console.warn(
			`closeLoop: unknown outcome "${outcome}" — treating as escalate (fail safe).`,
		);
		outcome = "escalate";
	}
	...
	if (outcome === "fix" && !green) {
		gateReason = tests
			? `tests are not green (${tests.passed}/${tests.total})`
			: "no test results were provided";
		console.warn(
			`closeLoop: not merging — ${gateReason}. Downgrading fix → escalate for human review.`,
		);
		outcome = "escalate";
	}
	...
	if (outcome === "fix") {
		...
		if (!scan.passed) {
			...
			console.warn(
				`closeLoop: not merging — ${opseraReason}. Downgrading fix → escalate for human review.`,
			);
			outcome = "escalate";
		}
	}
	const opseraDetail = opseraFindings
		.map((f) => f.message || f.rule)
		.filter(Boolean)
		.join("; ");
	...
	if (outcome === "escalate") {
		if (opseraReason) {
			...
			whyText =
				result.why ||
				violationDesc ||
				"Couldn't auto-fix this safely — needs human review.";
		}
	} else if (outcome === "allow") {
		whyText =
			result.why || violationDesc || "Reviewed — compliant, no action needed.";
	} else {
	...
	const mergeInput = {
		...
		violation: {
			rule: opseraReason
				? "Opsera compliance gate"
				: violation.rule || violationDesc || "(unspecified)",
			file: violation.file,
			line: violation.line,
			description: opseraReason
				? opseraDetail || opseraReason
		...
		sandbox: {
			testsPassed: outcome === "fix" ? true : Boolean(green),
			passed: tests?.passed,
			total: tests?.total,
		},
	};
	...
	return {
		...
		slack: out.slack,
		...(gateReason
			? { downgradedFrom: "fix", downgradeReason: gateReason }
			: {}),
		...(opseraReason
			? {
					downgradedFrom: "fix",
					downgradeReason: opseraReason,
					opsera: { passed: false, findings: opseraFindings },
				}
...
function buildDiff(result) {
	...
	const minus = String(before ?? "")
		.split("\n")
		.map((l) => `- ${l}`);
	const plus = String(after ?? "")
		.split("\n")
		.map((l) => `+ ${l}`);
