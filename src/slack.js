...
export async function notifySlack({
	...
	if (outcome === "allow") {
		...
		return sendSlack(
			text,
			buildAllowBlocks({ summary, checked, prUrl, location }),
		);
	}
...
function buildEscalateBlocks({
	...
	const blocks = [
		{
			...
			text: {
				type: "plain_text",
				text: `🚨 Human review needed${sev ? ` · ${sev}` : ""}`,
				emoji: true,
			},
		},
		{
			...
			text: {
				type: "mrkdwn",
				text: `*Couldn't auto-fix this safely.* ${summary}`,
			},
		},
	];
	...
	if (adds(whyCantFix, summary)) {
		blocks.push({
			type: "section",
			text: { type: "mrkdwn", text: `*Why I couldn't auto-fix it:* ${whyCantFix}` },
		});
	}
	...
	if (change) {
		blocks.push({
			type: "section",
			text: { type: "mrkdwn", text: `*Offending code:*\n${codeBlock(change)}` },
		});
	}
	...
	if (mention)
		blocks.push({
			type: "section",
			text: { type: "mrkdwn", text: `${mention} — please take a look.` },
		});
...
function buildMergedBlocks({
	...
	if (typeof testsTotal === "number") {
		...
		meta.push(
			`🧪 ${passed}/${testsTotal} tests passed · verified in an isolated sandbox`,
		);
	}
