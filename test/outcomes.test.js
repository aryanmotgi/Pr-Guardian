...
const commentFor = (gh, pr) =>
	gh.find((c) => new RegExp(`/issues/${pr}/comments$`).test(c.url));
...
test("ESCALATE: @-mentions a human, alerts (not the green card), and does NOT merge", async () => {
	...
	assert.equal(
		actions.elements[0].style,
		"danger",
		"alert-styled (red) button",
	);
	...
	assert.match(
		msg.text,
		/🔴 HIGH/,
		"tags the alert with a severity for triage",
	);
