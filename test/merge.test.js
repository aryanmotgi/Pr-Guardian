...
function installRecorder() {
  ...
  const fetch = async (url, opts = {}) => {
    ...
    if (/\/pulls\/\d+\/merge$/.test(url)) {
      ...
      data = { id: 1, html_url: "https://github.com/acme/acme-payments/pull/42#issuecomment-1" };
      status = 201;
    }
...
const mergeCall = (calls) => calls.find((c) => /\/pulls\/\d+\/merge$/.test(c.url));
const commentCall = (calls) => calls.find((c) => /\/issues\/\d+\/comments$/.test(c.url));
...
test("fix → merges the PR and posts a receipt with correct GitHub API calls", async () => {
  ...
  console.log("[fix] github calls:", calls.map((c) => `${c.method} ${c.url}`));
  ...
  assert.match(comment.url, /\/repos\/acme\/acme-payments\/issues\/42\/comments$/);
...
test("safety gate → refuses to merge a fix whose tests did not pass", async () => {
  installRecorder();
  const bad = { ...fixtures.fix, sandbox: { ...fixtures.fix.sandbox, testsPassed: false } };
